...
def print_parameters(model, wandb_log = False):
	""" Print the number of parameters of the model """
	print('\nTotal number of parameters: %d' % (sum(p.numel() for p in model.parameters() if p.requires_grad)))
	for name, sub_modules in model.named_children():
		num_params = sum(p.numel() for p in sub_modules.parameters() if p.requires_grad)
		print(f"{name} params: {str(num_params)}")
	print(" ")

	if wandb_log:
		wandb.log({'total_params': sum(p.numel() for p in model.parameters() if p.requires_grad)})
		for name, sub_modules in model.named_children():
			num_params = sum(p.numel() for p in sub_modules.parameters() if p.requires_grad)
